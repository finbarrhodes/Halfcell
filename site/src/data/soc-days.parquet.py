...
The backtester compares real days against the average week. That needs every
day's path, though not at full precision: state of charge and the range the FR
contracts required are stored as whole steps of 1/250 (0.4 percentage points)
and the date as days since 1970, which keeps ~260k rows small. The browser
measures each day's distance from the average for whichever strategy and date
range is selected.
