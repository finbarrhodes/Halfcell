...
perfect foresight with no floor subtracted, on which the model scores 80.5%. On a stacked battery
that says little, because reusing the last complete day's prices already scores 77.8%: most of the
...
advantage is trading, while the model earns most of its lead through response (+£2.2k, against
+£0.8k at trading): it values each block's arbitrage more accurately when the offers are made, and
